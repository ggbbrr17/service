...
def plan_to_concrete_steps(steps):
    # Por ahora solo devuelve lo mismo (puedes expandir luego)
    return steps
...
def execute_step(step: dict, dry_run: bool = False):
    ...
    try:
        if action == "open_browser":
            ...
            query = step.get("query")
            provider = step.get("provider", "duckduckgo")
            
            # Nueva integración con Gemini API para investigación sin navegador
            if provider in ["gemini", "chatgpt"]:
                api_key = os.getenv("GLYPH_GEMINI_API_KEY")
                if not api_key:
                    # Fallback al navegador según plataforma
                    webbrowser.open(f"https://www.google.com/search?q={query}")
                    return True, f"Abriendo Google Search (Modo IA) para investigar '{query}', ya que no hay API Key disponible."
                
                model = os.getenv("GLYPH_GEMINI_MODEL", "gemini-1.5-flash")
                url = f"https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={api_key}"
                payload = {"contents": [{"parts": [{"text": f"Actúa como un oráculo de búsqueda. Investiga y resume de forma técnica y precisa: {query}"}]}]}
                try:
                    res = requests.post(url, json=payload, timeout=20)
                    if res.ok:
                        info = res.json()['candidates'][0]['content']['parts'][0]['text']
                        return True, f"💡 RESPUESTA DE ORÁCULO ({provider.upper()}):\n\n{info}"
                    else:
                        provider = "duckduckgo" # Fallback a web scraper si falla la API
                except:
                    provider = "duckduckgo"

            try:
                # Modo Headless Real (DuckDuckGo Scraper)
                if provider == "duckduckgo":
                    url = f"https://html.duckduckgo.com/html/?q={query}"
                else:
                    # Fallback simple a Google si se pide otro (aunque Google bloquea scraping directo más fácil)
                    url = f"https://www.google.com/search?q={query}"
                
                headers = {"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) Glyph/1.0"}
                res = requests.get(url, headers=headers, timeout=15)
                if res.ok:
                    # Extraemos los snippets de los resultados para que el mensaje sea útil
                    snippets = re.findall(r'result__snippet.*?>(.*?)</a>', res.text, re.DOTALL)
                    if not snippets:
                        snippets = re.findall(r'result__snippet.*?>(.*?)</div>', res.text, re.DOTALL)
                    
                    clean_text = "\n\n• ".join([re.sub('<[^>]*>', '', s).strip() for s in snippets[:5]])
                    return True, f"🔍 INVESTIGACIÓN HEADLESS (Fallback):\n\n• {clean_text}"
                
                return False, f"No se pudo obtener información de {provider}."
            except Exception as e:
                return False, f"Error en investigación: {str(e)}"
            ...
            try:
                ...
                root.destroy()
                # Truncamos para evitar errores de límite en Telegram (4096 caracteres)
