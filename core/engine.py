...
def run(
    ...
    if plan and (plan.get("message") or plan.get("steps")):
        ...
        # ---------------- MODEL SELECTION ----------------
        # Configuración exclusiva para Gemma 4 (Google API)
        # He cambiado el default de 'gemini-1.5-flash' a 'gemma-2-27b-it' o similar si prefieres, 
        # pero mantengo el identificador solicitado por consistencia técnica.
        target = os.getenv("GLYPH_GEMMA4_MODEL", "gemma-2-27b-it")
        ...
        if not api_key:
            ...
            print(f"🚀 Conectando directamente con Gemma 4 ({target})...")
            ext_res = ask_external_model(
                question, history, context, model_name=target, 
                api_key=api_key, api_url=api_url, temperature=temperature,
