...
def run(
    ...
    if plan and (plan.get("message") or plan.get("steps")):
        ...
        # ---------------- MODEL SELECTION ----------------
        # Configuración para Deep Research (Gemini 2.0 Flash Thinking)
        # Este modelo permite razonamiento profundo y búsqueda web integrada.
        target = os.getenv("GLYPH_GEMINI_MODEL", "gemini-2.0-flash-thinking-preview")
        api_key = os.getenv("GLYPH_GEMINI_API_KEY")
        # Usamos el endpoint nativo de Google para aprovechar las herramientas de búsqueda (Grounding)
        ...
        if not api_key:
            ...
            print(f"🚀 Iniciando Deep Research con {target}...")
            ext_res = ask_external_model(
                ...
                use_google_search=True # Habilita la investigación web autónoma
            )
