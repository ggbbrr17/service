...
def run(
    ...
    if plan and (plan.get("message") or plan.get("steps")):
        ...
        # ---------------- MODEL SELECTION ----------------
        # Usamos Gemini 2.0 Flash vía API de Google (más estable que el thinking-preview)
        target = os.getenv("GLYPH_GEMINI_MODEL", "gemini-2.0-flash")
        ...
        if not api_key:
            ...
            ext_res = ask_external_model(
                ...
                use_google_search=False # Eliminado el Deep Research por petición
            )
