...
def run(question: str, dry_run: bool = False, history: str = "", depth: int = 0, temperature: float = 0.0, is_user: bool = False) -> dict:
    ...
    if plan:
        ...
        cloud_models = ["llama3", "deepseek", "openrouter", "gemma", "gemma4", "cerebras", "minimax-m2.7:cloud"]
        ...
        if active_model in cloud_models:
            ...
            if active_model == "llama3":
                ...
                target = os.getenv("GLYPH_GEMMA4_MODEL", "gemini-1.5-flash")
            else:
                target = active_model
            ...
            if not api_key:
                print(f"❌ ERROR CRÍTICO: Falta la variable '{env_var_name}' para usar {active_model.upper()}.")
        ...
        if (not plan_text or "ERROR_CONNECTION" in plan_text or "404" in plan_text or "429" in plan_text) and active_model not in ["tinyllama", "gemma4"]:
            ...
            fallback_chain = [
                {"slug": "gemma4", "name": "Gemma 4 (Google Flash)", "model": os.getenv("GLYPH_GEMMA4_MODEL", "gemini-1.5-flash"), "key": gemini_key, "url": "https://generativelanguage.googleapis.com/v1beta/openai/chat/completions"},
